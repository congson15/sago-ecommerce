...
import { cartItemsCountSelector } from '../../pages/CartPage/selectors';
...
const routes = [
  ...
  {
    label: 'Men',
    path: '/products?category=Men&sort=asc',
  },
  {
    label: 'Women',
    path: '/products?category=Women&sort=asc',
  },
  {
    label: 'Order Status',
    path: '/orders',
...
export function Header(props: HeaderProps) {
  ...
  const [searchInput, setSearchInput] = React.useState(true);
  const [mdOptionsToggle, setMdOptionsToggle] = React.useState(true);
  ...
  const handleSearch = () => {
    if (keyword.trim() === '') {
      console.log(' Please enter some keyword to search.');
      return false;
    }
  ...
  return (
    <div>
      <div className="bg-303030 h-7">
        <ul className="flex flex-row-reverse">
          <li className="flex group items-center">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-50 group-hover:text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
            </svg>
            <Link to="/login" className="mx-2 text-gray-50 group-hover:text-purple-400">Login</Link>
          </li>
        </ul>
      </div>
        ...
        <div className="relative">
          <div id="md-searchbar" className={`${mdOptionsToggle ? "hidden" : "flex"} bg-white dark:bg-gray-900 lg:hidden py-5 px-6 items-center justify-between`}>
            ...
            <div className="space-x-6">
              <button aria-label="view favourites" className="text-gray-800 dark:text-white ">
          ...
          <div className="dark:bg-gray-900 bg-gray-50 px-6 py-9">
            <div className="container mx-auto flex items-center justify-between">
              <Link to="/">
                <div className="w-full uppercase text-transparent cursor-pointer bg-clip-text bg-gradient-to-r from-purple-500 to-red-400 md:text-center text-2xl font-semibold">
                  Sago
                </div>
              </Link>
              <ul className="hidden w-8/12 md:flex items-center justify-center space-x-8">
                {routes.map((route, index) => <li>
                  <Link to={route.path} key={index} className="dark:text-white text-base text-gray-800 ">
                    {route.label}
                  </Link>
                </li>)}
              </ul>
              <div className="md:w-2/12 justify-end flex items-center space-x-4 xl:space-x-8">
                <div className="hidden lg:flex items-center">
                  <button onClick={() => setSearchInput(!searchInput)} aria-label="search items" className="text-gray-800 dark:hover:text-gray-300 dark:text-white">
                  ...
                  <input type="text" placeholder="Search" className={` ${searchInput ? "hidden" : ""} dark:bg-gray-900 dark:placeholder-gray-300 text-gray-600 rounded ml-1 border border-transparent border-gray-400 focus:outline-none px-1`} />
                </div>
                <Link to="/cart" className="hidden lg:flex items-center space-x-4 xl:space-x-8 relative">
                  <button className="text-gray-800 dark:hover:text-gray-300 dark:text-white">
                  ...
                  {cartItemsCount !== 0 ?
                    <span className="absolute text-center -top-2 -right-3 px-1.5 w-6 py-1 text-xs font-bold leading-none text-red-100 transform bg-red-600 rounded-full">
          ...
          <div className={`${showMenu ? "flex" : "hidden"} absolute dark:bg-gray-900 z-10 inset-0 md:hidden bg-white flex-col h-screen w-full`}>
            ...
            <div className="h-full flex items-end">
              <ul className="flex flex-col space-y-8 bg-gray-50 w-full py-10 p-4 dark:bg-gray-800">
                <li>
                  <Link to="/cart" className="dark:text-white text-gray-800 flex items-center space-x-2 ">
                    <div>
      ...
      <div className="bg-eee flex h-12 justify-center">
        <p className="flex items-center text-5b5b5b tracking-normal leading-5 justify-between">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-11 w-11 text-purple-500 mr-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path d="M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0z" />
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0" />
          </svg>
          <strong className="font-bold">Free delivery</strong>: Get free standard delivery on every order with Shoe Store
        </p>
      </div>
    </div >
  );
